...
//        int ApplicationType_Admin = 2;
	...
	addbaoxiudan(data) {
	    return this._post2('/sys/maintain/add', data);
	}
	getbaoxiudan(data) {
	    return this._get2('/sys/maintain/list', data);
	}
    ...
    async _get2(path, data = {}, rawResponse = false, rawResponseData = false) {
         let response;
         path = Config.ORGANIZATION_SERVER2 + path;
	...
	async _post2(path, data = {}, rawResponse = false, rawResponseData = false) {
	    let response;
	    path = Config.ORGANIZATION_SERVER2 + path;
	    response = await axios.post(path, data, {
	        transformResponse: rawResponseData ? [data => data] : axios.defaults.transformResponse, headers: {
